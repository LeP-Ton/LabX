...
- 修复提交 `dfad166` 已推送到 `main`。
- 质量检查工作流 `33759904364` 已通过，E2E 结果为 13 项通过、1 项按项目配置跳过。
- GitHub Pages 工作流 `33759904311` 已通过，修复后的站点已重新部署。
...
- 是否通过：通过；桌面端与移动端 E2E 均未再出现 strict mode 冲突。
...
- 是否通过：通过；断言已在当前三项目数据下通过，并保持对 1、2 项数据的兼容语义。
